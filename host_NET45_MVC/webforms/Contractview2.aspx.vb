...
Partial Class contractView_ApprovalMatrixPTM3SSTZORT
    ...
    Protected Sub GridViewShowContract_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridViewShowContract.RowCommand
        ...
        If (e.CommandName = "OpenPDF") Then
            ...
            Session.Remove("fileManagerOpenPDFContract_Link")
            Session.Remove("fileManagerOpenPDFContract_Editable")

            Session("fileManagerOpenPDFContract_Link") = LinkToFile
            Session("fileManagerOpenPDFContract_Editable") = 1
    ...
    Protected Sub GridViewShowContract_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridViewShowContract.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Else
                ...
                ' THIS SECTION MIGHT BE NEEDED BUT I DISABLED. GridviewApprovalStatus.Enabled = False can be tricky, if needed activated this somewhere else
                ...
                '    ' show message to user about PO status and its value
                '    If DirectCast(e.Row.FindControl("LabelPoStatus"), Label) IsNot Nothing Then
                '        DirectCast(e.Row.FindControl("LabelPoStatus"), Label).Text = BodyTexts.Ref("ip6sNNqRYUCj0UWbr0x+/A") + DataBinder.Eval(e.Row.DataItem, "Scenario").ToString
                ...
                '    ' show message to user about PO status and its value
                '    If DirectCast(e.Row.FindControl("LabelPoStatus"), Label) IsNot Nothing Then
                '        DirectCast(e.Row.FindControl("LabelPoStatus"), Label).Text = BodyTexts.Ref("19vzkaMbPUSTTRuwgcNBpg") + DataBinder.Eval(e.Row.DataItem, "Scenario").ToString
                '        DirectCast(e.Row.FindControl("LabelPoStatus"), Label).CssClass = "label label-danger arrowed-in arrowed-in-right"
                '    End If
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If PanelClientContractAdditionalDetails IsNot Nothing Then
                If DataBinder.Eval(e.Row.DataItem, "ProjectID") = 999 Then
                    ...
                    If Not IsDBNull(DataBinder.Eval(e.Row.DataItem, "Penalties")) Then
                        If DataBinder.Eval(e.Row.DataItem, "Penalties") = True Then
                            LiteralPenaltyClient.Text = BodyTexts.Ref("2TUx46Nv1UeQLerDF4GQjA")
                        Else
                            LiteralPenaltyClient.Text = BodyTexts.Ref("sl3la1Y+vUmYXJQRX9Mosw")
                        End If
                    End If
                Else
    ...
    Protected Sub GridViewShowAddendum_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Else
                ...
                ' THIS PART DISABLED. GridviewApprovalStatusAddendum_.Enabled = False might be tricky. You may need to enable somewhere else
                'If ContractView.AddendumReadyToTurnPo(DataBinder.Eval(e.Row.DataItem, "AddendumID")) _
                '  And _NewGeneration = True Then
                '    ' --------------------------------------------------------------------------------------TIME TO TURN PO
                    ...
                    If FileUploadAktOfWorkAddendum.HasFile Then
                        If System.IO.Path.GetExtension(FileUploadAktOfWorkAddendum.PostedFile.FileName) = ".pdf" Then
                            If Directory.Exists(Server.MapPath("~/CONTRACT/") + DDLProjectEdit.SelectedItem.ToString + "/") Then
                                ...
                                _GiveNotification.Gritter_Error(Page, BodyTexts.Ref("auG3R9xVU0e2V3RrFYwIOA"), FileUploadAktOfWorkAddendum.PostedFile.FileName + " " + BodyTexts.Ref("nT+75PL7wkutDMuL3dxlAQ"), "success")
                            Else
                                Dim UniqueString21 As String = Guid.NewGuid().ToString().GetHashCode().ToString("x")
                                Dim UniqueString22 As String = Guid.NewGuid().ToString().GetHashCode().ToString("x")
                                Directory.CreateDirectory(Server.MapPath("~/CONTRACT/") + DDLProjectEdit.SelectedItem.ToString)
    ...
    Private Sub contractView_ApprovalMatrixPTM3SSTZORT_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete

        If IsPostBack Then
            If Session("fileManagerOpenPDFContract_Link") IsNot Nothing Then

                HiddenFieldfileManager_Link.Value = Session("fileManagerOpenPDFContract_Link")
                HiddenFieldfileManager_Editable.Value = Session("fileManagerOpenPDFContract_Editable")

                Dim webFormCommon = New host_NET45_MVC.App_Code.WebFormCommon
                webFormCommon.processFileManager(Page, panelContainer, HiddenFieldfileManager_Link.Value, HiddenFieldfileManager_Editable.Value)
