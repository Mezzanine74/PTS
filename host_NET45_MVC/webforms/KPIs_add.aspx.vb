...
Partial Class KPIs_add
  ...
  Protected Sub FormViewKPIs_ItemInserting(sender As Object, e As System.Web.UI.WebControls.FormViewInsertEventArgs) Handles FormViewKPIs.ItemInserting
    ...
    If DropDownListProject.SelectedValue = 0 Then
      ...
      Exit Sub
    Else
      e.Values("idKPI") = DropDownListProject.SelectedValue
    End If

    Dim TextBoxAddNo As TextBox = FormViewKPIs.FindControl("TextBoxAddNo")
    If String.IsNullOrEmpty(TextBoxAddNo.Text) Then
      e.Values("AddNo") = Nothing
    Else
      e.Values("AddNo") = TextBoxAddNo.Text
    End If
    ...
    Else
      e.Values("RetentionTerms") = Convert.ToString(TextBoxRetentionTerms.Text)
    End If
...
FormViewKPIs.FindControl("TextBoxPenaltyTerms")
    ...
    Dim TextBoxCommentAddNo As TextBox = FormViewKPIs.FindControl("TextBoxCommentAddNo")
    If String.IsNullOrEmpty(TextBoxCommentAddNo.Text) Then
      e.Values("NoteAddNo") = Nothing
    Else
      e.Values("NoteAddNo") = Convert.ToString(TextBoxCommentAddNo.Text)
    End If
  ...
  Protected Sub FormViewKPIs_ItemUpdating(sender As Object, e As System.Web.UI.WebControls.FormViewUpdateEventArgs) Handles FormViewKPIs.ItemUpdating
    ...
    If DropDownListProject.SelectedValue = 0 Then
      ...
      Exit Sub
    Else
      e.NewValues("idKPI") = DropDownListProject.SelectedValue
    End If

    Dim TextBoxAddNo As TextBox = FormViewKPIs.FindControl("TextBoxAddNo")
    If String.IsNullOrEmpty(TextBoxAddNo.Text) Then
      e.NewValues("AddNo") = Nothing
    Else
      e.NewValues("AddNo") = TextBoxAddNo.Text
    End If
...
FormViewKPIs.FindControl("TextBoxPenaltyTerms")
    ...
    Dim TextBoxCommentAddNo As TextBox = FormViewKPIs.FindControl("TextBoxCommentAddNo")
    If String.IsNullOrEmpty(TextBoxCommentAddNo.Text) Then
      e.NewValues("NoteAddNo") = Nothing
    Else
      e.NewValues("NoteAddNo") = Convert.ToString(TextBoxCommentAddNo.Text)
    End If
  ...
  Protected Sub ChangeModeAccordingly(ByVal FormViewToManuplate As FormView)
    ...
    If Page.User.IsInRole("AddClientData") Then
      ...
      If ProjectKPIExist(Request.QueryString("ProjectID"), Request.QueryString("AddNo")) = True Then
  ...
  Protected Function GetProjectCurrency(ByVal ProjectID_ As Integer) As String
    ...
    Protected Function ProjectKPIExist(ByVal ProjectID_ As Integer, ByVal AddNo_ As Integer) As Boolean
        Dim con As SqlConnection = ConnectionStringsPTS.GetConnectionStringMain
        con.Open()
        Dim sqlstring As String = " SELECT     COUNT(dbo.Table_KPIs_add.idKPI) AS CountOf " +
                                              " FROM         dbo.Table_KPIs INNER JOIN " +
                                              "                       dbo.Table_KPIs_add ON dbo.Table_KPIs.idKPI = dbo.Table_KPIs_add.idKPI " +
                                              " WHERE     (dbo.Table_KPIs.ProjectID = @ProjectID) AND (dbo.Table_KPIs_add.addNo = @AddNo) "
        ...
        Dim ProjectID As SqlParameter = cmd.Parameters.Add("@ProjectID", System.Data.SqlDbType.Int)
        ProjectID.Value = ProjectID_
        Dim AddNo As SqlParameter = cmd.Parameters.Add("@AddNo", System.Data.SqlDbType.Int)
        AddNo.Value = AddNo_
  ...
  Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
    If Not IsPostBack Then
      ...
      SqlDataSourceKPIs.UpdateParameters("NoteAddNo").DefaultValue = Nothing
      ...
      SqlDataSourceKPIs.InsertParameters("NoteAddNo").DefaultValue = Nothing
