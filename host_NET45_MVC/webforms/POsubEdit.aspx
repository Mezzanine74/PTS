<%@ Page Title="" Language="VB" MasterPageFile="~/site.master" AutoEventWireup="false" CodeFile="POsubEdit.aspx.vb" Inherits="POsubEdit" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
  ...
  <asp:SqlDataSource ID="SqlDataSourcePoInfo" runat="server" 
    ...
    SelectCommand="SELECT     dbo.Table2_PONo.PO_No, RTRIM(dbo.Table6_Supplier.SupplierName) AS SupplierName, RTRIM(dbo.Table2_PONo.Description) AS Description, 
                      ...
                      RTRIM(dbo.Table2_PONo.PO_Currency) AS PO_Currency, SUM(CASE WHEN CollectedValue IS NULL THEN 0 ELSE CollectedValue END) 
                      AS CollectedValue
  ...
  <asp:Panel ID="panelHidden" runat="server" CssClass="hidepanel" >
    ...
    FROM         dbo.Table2_PONo INNER JOIN
      ...
      <asp:SqlDataSource ID="SqlDataSourceTotalCollectedDocuments" runat="server" 
        ConnectionString="<%$ ConnectionStrings:MERCURYConnectionString %>" 
        SelectCommand=" SELECT     (CASE WHEN SUM(dbo.Table5_PayLog.CollectedValue) IS NULL THEN 0 ELSE SUM(dbo.Table5_PayLog.CollectedValue) END) AS CollectedValue
    FROM         dbo.Table2_PONo INNER JOIN
